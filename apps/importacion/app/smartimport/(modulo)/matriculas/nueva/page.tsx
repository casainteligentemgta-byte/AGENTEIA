...
export default async function NuevaFichaMatriculaPage({ searchParams }: Props) {
  const user = await getUser();
  if (!user) {
    redirect("/smartimport/login?redirectTo=/smartimport/matriculas/nueva");
  }
      ...
      <div className="mx-auto max-w-2xl">
        <div className="mb-6 flex items-center gap-3">
          <Link
            href={
              expediente
                ? `/smartimport/matriculas?expediente=${encodeURIComponent(expediente)}`
                : "/smartimport/matriculas"
            }
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-950/40 p-5 sm:p-6">
          <MatriculaFichaForm
            ...
            afterCreateHref={(id) =>
              expediente
                ? `/smartimport/matriculas/${id}?expediente=${encodeURIComponent(expediente)}`
                : `/smartimport/matriculas/${id}`
            }
          />
        </div>
      </div>
