...
export function EstadoVisualDisplay({ estadoVisual }: Props) {
  const fotosExterior = ESTADO_VISUAL_VISTAS.filter((v) => v !== "tablero")
    .map((vista) => estadoVisual.fotos.find((f) => f.vista === vista) ?? { vista, trazos: [] })
    .filter((f) => Boolean(f.url || f.path));

  const fotoTablero = estadoVisual.fotos.find(
    (f) => f.vista === "tablero" && Boolean(f.url || f.path)
  );
