...
import type { ExpedienteEnlace } from "@/lib/importacion/expediente-enlace";

type Props = {
  fichaId: string;
  expedientes: ExpedienteEnlace[];
...
export function FichaAsignarExpediente({
  fichaId,
  expedientes,
  preselectVehiculoId,
  assignAction,
  ...
  const asignados = useMemo(
    () => expedientes.filter((e) => e.fichaId === fichaId),
    [expedientes, fichaId]
  );
  const disponibles = useMemo(() => {
    const q = query.trim().toLowerCase();
    return expedientes
      .filter((e) => e.fichaId !== fichaId)
      .filter((e) => {
        if (!q) return true;
        const hay = `${e.codigoExpediente} ${e.marca ?? ""} ${e.modelo ?? ""} ${e.detalle ?? ""}`;
        return hay.toLowerCase().includes(q);
      });
  }, [expedientes, fichaId, query]);

  function asignar(id: string) {
    setError(null);
    setMessage(null);
    startTransition(async () => {
      const result = await assignAction({ fichaId, vehiculoId: id });
      if (!result.success) {
        setError(result.error);
        return;
      }
      setMessage("Expediente enlazado a esta ficha");
      setVehiculoId("");
      router.refresh();
    });
  }
    ...
    <section className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Al asignar se copian los datos de esta ficha al expediente.
        </p>
      </div>

      {asignados.length === 0 ? (
        <p className="text-sm text-zinc-500">Aún no hay expedientes enlazados.</p>
      ) : (
