...
export default async function SegurosPage({ searchParams }: Props) {
  const user = await getUser();
  if (!user) redirect("/smartimport/login?redirectTo=/smartimport/seguros");
  ...
  const listed = await listSegurosAction();
  const fichas = listed.success ? listed.fichas : [];
  const expediente = searchParams.expediente?.trim() || "";
  const nuevaHref = expediente
    ? `/smartimport/seguros/nueva?expediente=${encodeURIComponent(expediente)}`
    : "/smartimport/seguros/nueva";
      ...
      <div className="mx-auto max-w-2xl">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h1 className="text-xl font-semibold text-zinc-50">Fichas de seguro</h1>
          </div>
          <Link
            href={nuevaHref}
          ...
          <ul className="space-y-2">
            {fichas.map((f) => {
              const href = expediente
                ? `/smartimport/seguros/${f.id}?expediente=${encodeURIComponent(expediente)}`
                : `/smartimport/seguros/${f.id}`;
              return (
                <li key={f.id}>
                  <Link
                    href={href}
                  ...
                  >
                    <p className="text-sm font-semibold text-zinc-50">{f.aseguradora}</p>
                    <p className="mt-0.5 font-mono text-xs text-zinc-400">
                      {f.numeroPoliza || "Sin póliza"}
                    </p>
